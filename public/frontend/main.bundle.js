...
var AdminSchedulesComponent = (function () {
    function AdminSchedulesComponent(_httpService) {
        ...
        this.urls = {
            retrieve_schedule_url: __WEBPACK_IMPORTED_MODULE_2__config_config__["a" /* BACKEND_SERVER_URL */] + "api/admin/schedule/retrieve_by_date"
        };
        this.calendarInfo = {
            cur_date_str: "",
            cur_year: "",
    ...
    AdminSchedulesComponent.prototype.setCurentDateForCalendar = function () {
        ...
        this.calendarInfo.cur_date_str = this.convertMonthFormat(d.getMonth()) + " " + d.getFullYear();
    };
    ...
    AdminSchedulesComponent.prototype.addDaysToCalendar = function () {
        ...
        var temp = this.convertDate(firstDay);
        var url = this.urls.retrieve_schedule_url + "?date=" + temp;
        console.log(temp);
        this.addDayToCalendar(url, firstDay, lastDay);
    };
    AdminSchedulesComponent.prototype.addDayToCalendar = function (url, firstDay, lastDay) {
        var _this = this;
        this._httpService.sendGetRequestWithParams(url)
            ...
            if (response['state'] == 'success') {
                ...
                var cnt = Object.keys(response['data']).length;
                var html = void 0;
                if (firstDay.getDate() < 10) {
                    html = "<div class='col-md-1 fix-height'><h3>0" + firstDay.getDate() + "</h3><p>";
                }
                else {
                    html = "<div class='col-md-1 fix-height'><h3>" + firstDay.getDate() + "</h3><p>";
                }
                if (cnt > 1) {
                    html += cnt + " buses</p></div>";
                }
                else {
                    html += cnt + " bus</p></div>";
                }
                ...
                var temp = _this.convertDate(firstDay);
                var url_1 = _this.urls.retrieve_schedule_url + "?date=" + temp;
                console.log(temp);
                if (firstDay <= lastDay) {
                    _this.addDayToCalendar(url_1, firstDay, lastDay);
                }
            }
        });
    };
