...
<div class="td likep" width="674"><br>
    ...
    <br>
    This email is regarding the {{$reservation['old']['Seats']}} 
    @if ($reservation['old']['Seats'] > 1)
    seats
    @else
    seat
    @endif
     you had with us from {{$oldStop['address']}} at {{$oldTime['time']}} on {{$reservation['old']['date']}}. We are hereby giving you an additional {{$reservation['Seats']}} 
    ...
    <br>
    Your reservation number (<b>{{$reservation['old']['id']}}</b>) is not affected by this action. <br>
</div>
@endsection
