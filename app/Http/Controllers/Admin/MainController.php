...
/*            if ($trans_id = addAuthorizeNetLink($reservation)) {
    ...
    /*            if ($trans_id = addAuthorizeNetLink($reservation)) {
            ...
            $username = Auth::user()->full_name;
            $now = date('Y/m/d g:i A');
            $note1 = 'Had ' . $resReservation['Seats'] . ' seat' . ($resReservation['Seats'] > 1 ? 's' : '') . ' (leaving from ' . $fromStop . '. at ' . $fromTime . ' on ' . $fromDate . '), Admin (' . $username . ') gave them additional ' . $resReservation['Seats'] . ' seat' . ($resReservation['Seats'] > 1 ? 's' : '') . ' as a complimentary ' . $now . '. (Reason for this action: ' . $inputBody . '.). That Complimentary # is ' . $resReservation['id'] . '.';
            if ($oldReservation['Note'] != '') {
                $note1 = "\n" . $note1;
            }
            $oldReservation['Note'] .= $note1;
            ...
            $this->sendMail_Reservation_Complimentary($resReservation->toarray(), $oldReservation->toarray(), $inputSubject, $inputBody);
    ...
    public function complimentaryOneReservations(Request $request){
        $input = $request->only(['reservations', 'subject', 'body']);
        $inputReservations = $input['reservations'];
        $inputSubject = $input['subject'];
        $inputBody = $input['body'];
    ...
    public function sendMail_Reservation_Complimentary($reservation, $oldReservation, $subject, $body) {
        $reservation['old'] = $oldReservation;
        $reservation['mailSubject'] = $subject;
